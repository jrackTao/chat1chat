import { ComponentClass } from 'react'
import Taro, { Component, Config } from '@tarojs/taro'
import { View, Text, ScrollView } from '@tarojs/components'
import SearchInput from '@/components/SearchInput'
import BoxCard from '@/components/BoxCard'
import TabBtn from '@/components/TabBtn'
import './index.scss'
import { AtNavBar, AtAvatar, AtInput, AtBadge } from 'taro-ui'
import '../style.scss';

type PageStateProps = {}

type PageDispatchProps = {}

type PageOwnProps = {}

type PageState = {}

type IProps = PageStateProps & PageDispatchProps & PageOwnProps

interface Index {
  props: IProps;
}

class Index extends Component {
  constructor(props) {
    super(props)
    this.windowHeight = Taro.getSystemInfoSync().windowHeight
  }

  state = {
    tabOptions: [{
      id: 0,
      text: '好友',
    }, {
      id: 1,
      text: '群聊',
    }, {
      id: 2,
      text: '设备',
    }, {
      id: 3,
      text: '通讯录',
    }, {
      id: 4,
      text: '公众号',
    }],
    list: [
      {
        name: 'Golang社区',
        avatar: 'https://jdc.jd.com/img/200',
        text: '老朱很帅，大家都知道',
        time: '下午5:24',
        hint: 2,
      },
      {
        name: 'Golang社区',
        avatar: 'https://jdc.jd.com/img/200',
        text: '老朱很帅，大家都知道',
        time: '下午5:24',
        hint: 2,
      },
      {
        name: 'Golang社区',
        avatar: 'https://jdc.jd.com/img/200',
        text: '老朱很帅，大家都知道',
        time: '下午5:24',
        hint: 2,
      },
      {
        name: 'Golang社区',
        avatar: 'https://jdc.jd.com/img/200',
        text: '老朱很帅，大家都知道',
        time: '下午5:24',
        hint: 2,
      },
      {
        name: 'Golang社区',
        avatar: 'https://jdc.jd.com/img/200',
        text: '老朱很帅，大家都知道',
        time: '下午5:24',
        hint: 2,
      },
      {
        name: 'Golang社区',
        avatar: 'https://jdc.jd.com/img/200',
        text: '老朱很帅，大家都知道',
        time: '下午5:24',
        hint: 2,
      },
      {
        name: 'Golang社区',
        avatar: 'https://jdc.jd.com/img/200',
        text: '老朱很帅，大家都知道',
        time: '下午5:24',
        hint: 2,
      },
    ]
  }

  goToChat = () => {
    Taro.navigateTo({
      url: '/pages/chat/index',
    })
  }

  tabOnChange = option => {
    console.log(option);
  }

  onClickRgIconSt = () => {
    Taro.navigateTo({
      url:'/pages/add/index'
    })
  }

  render() {
    const { list, tabOptions } = this.state;
    return (
      <View>
        <AtNavBar
          onClickRgIconSt={this.onClickRgIconSt}
          rightFirstIconType="iconfont icon-add-friend white-color"
          color='#000'
          title='联系人'
        >
        </AtNavBar>
        <SearchInput></SearchInput>
        <TabBtn options={tabOptions} onChange={this.tabOnChange}></TabBtn>
        <ScrollView
          style={{
            backgroundColor: '#fff',
            height: `calc(${this.windowHeight}px - ${Taro.pxTransform(400)})`
          }}
          scrollY>
          {
            list.map(item =>
              <View className="message-i" onClick={this.goToChat}>
                <BoxCard data={item}>
                </BoxCard>
              </View>
            )
          }
        </ScrollView>
      </View >
    )
  }
}

export default Index as ComponentClass<PageOwnProps, PageState>